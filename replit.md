# Chan Corp Auth Panel

## Overview
A software licensing and authentication management panel with CHAN CORP KEYAUTH UI. Features role-based access control (Super Admin, Admin, Reseller). No public registration — accounts are created by Super Admin only. Includes a public client API compatible with KeyAuth-style client libraries.

## Recent Major Changes (April 2026)
- **New logo**: Rebranded to CC/CHAN CORP logo across login page and sidebar
...
## Credentials
- **Super Admin**: username `CHAN-SR`, password `vc3yge5f` (seeded on startup)

## Architecture
- **Frontend**: React + Vite + Tailwind CSS + shadcn/ui components — CC dark purple theme
